...
public class IntroToEnums {
	...
	public static void main(String[] args) {
		...
		String userinput = JOptionPane.showInputDialog("Type a state of matter");
		// 5. Iterate through the array and find what the user entered.
		// Hint: .name() or .toString
		for(int i = 0; i < statesofmatter.length; i++) {
			System.out.println(statesofmatter[i].getCelsiusTemp());
			if(statesofmatter[i].toString() .equals(userinput)) {
		...
		// int daysInMonth;
		//
		// private Months(int daysInMonth) {w
		// }
		...
		StatesOfMatter stateofmatter;
		Random rand = new Random();
		stateofmatter = StatesOfMatter.values()[rand.nextInt(3)];
		// 11. Print outs both of its temperatures.
		System.out.println(stateofmatter.getCelsiusTemp() + stateofmatter.convertToFarenheit(stateofmatter.getCelsiusTemp()));
		...
		case SOLID:
			...
			break;
		case LIQUID:
			System.out.println("Starbucks matcha with peach juice and coconutmilk");
			break;
		case GAS:
