...
public class Snake {
	...
	private boolean isNotOppositeDirection(Direction direction) {
		 ...
		 */
		if(currentDirection.toString() .equals("UP") ) {
			if(direction.toString() .equals("DOWN")) {
				return false;
			}
		}
		else if(currentDirection.toString() .equals("DOWN")) {
			if(direction.toString() .equals("UP")) {
				return false;
			}
		}
		else if(currentDirection.toString() .equals("RIGHT")) {
			if(direction.toString() .equals("LEFT")) {
				return false;
			}
		}
		else if(currentDirection.toString() .equals("LEFT")) {
	...
	public void resetLocation() {
		 ...
		 * the size.
		 */
		head = new SnakeSegment(locoobj,BODY_SIZE);
	...
	public boolean isOutOfBounds() {
		...
		if(head.getLocation().getY() > SnakeGame.HEIGHT || head.getLocation().getX() < 0) {
			return true;
		}
		return false;
	}
